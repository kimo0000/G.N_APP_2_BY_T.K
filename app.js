...
addOfficer.addEventListener("click", () => {
  let imgSrc = imgEl.src,
    nameValue = nameEl.value.toLowerCase().trim(),
    prenomValue = prenomEl.value.toLowerCase().trim(),
    ageValue = ageEl.value.toLowerCase().trim(),
    numberValue = numberEl.value.toLowerCase().trim(),
    lieuxValue = lieuxEl.value.toLowerCase().trim();
